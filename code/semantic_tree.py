...
from consts import Info, ROOT, NumberInfo
...
class SemanticTree:
    ...
    def parse_text_without_processing_less_features(self, parsed_text, pos, features):
        word_list = list(zip(list(parsed_text), list(pos), map(lambda feature: self.get_feature_dict(feature), list(features))))
        self.tree = {index: NumberInfo(word, pos, self.get_gender(feature_dict), self.get_tense(feature_dict),
                                 self.get_number(feature_dict), self.get_person(feature_dict))
                     for index, (word, pos, feature_dict) in enumerate(word_list)}
        self.parsed_text = parsed_text
